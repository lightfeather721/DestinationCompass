...
- 支持百度地图步行/骑行路线规划，可按距离最短或时间最快显示路线
- 规划完成后自动收起地点卡片，并在地图上显示路线摘要
- 开启实时导航后路线状态保存在 ViewModel 中，切换应用页面后仍可恢复显示
- 支持实时导航开关，移动超过 5 米且间隔不少于 3 秒时自动重新规划路线
- 地图路线摘要卡片右侧提供“实时导航/结束导航”快捷按钮
- 未预先规划路线时也可直接开启实时导航，应用会自动规划并进入导航状态
- 地图路线摘要卡片使用实时背景模糊的磨砂玻璃效果
- 路线规划与实时导航期间始终保留用户位置到目标地点的方向连线
- 未开启实时导航时，离开地点页或移动超过 5 米会自动清除静态规划路线
